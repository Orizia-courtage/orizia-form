...
export async function readForm(request, requiredFields = ['numero_dossier', 'profil_prenom', 'profil_nom', 'profil_mail', 'profil_tel'], emailField = 'profil_mail') {
  ...
  if (!/^[^\s@<>]+@[^\s@<>]+\.[^\s@<>]+$/.test(payload[emailField])) throw new FormError('Email invalide.');
  return payload;
}
function secret(env) {
