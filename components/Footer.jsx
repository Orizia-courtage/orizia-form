  ...
  return (
    <footer style={{ backgroundColor: 'var(--orizia-accent)', color: 'var(--orizia-light)', padding: '60px 20px 30px' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 40, marginBottom: 40 }}>
        <div>
          <Image src="/images/Orizia_logo.jpg" alt="Orizia Courtage" width={130} height={60} style={{ objectFit: 'contain', filter: 'brightness(0) invert(1)', marginBottom: 16 }} />
          <p style={{ fontSize: '0.88rem', opacity: 0.8, lineHeight: 1.6 }}>Expert en Financement &amp; Patrimoine. Courtier certifié ORIAS.</p>
        </div>
        {[
          { title: 'Investir', links: [['Crowdfunding', '/investir/crowdfunding'], ['PERP / PER', '/investir/per'], ['Assurance Vie', '/investir/assurance-vie'], ['SCPI', '/investir/scpi']] },
          { title: 'Financer', links: [['Crédit immobilier', '/financer/credit-immobilier'], ['Regroupement de crédits', '/financer/regroupement-credits'], ['Prêt personnel', '/financer/pret-personnel']] },
          { title: 'Assurer', links: [['Assurance emprunteur', '/assurer/emprunteur'], ['Assurance habitation', '/assurer/habitation'], ['Assurance auto/moto', '/assurer/auto-moto']] },
        ].map(col => (
          <div key={col.title}>
            <h4 style={{ fontWeight: 800, fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 16, opacity: 0.6 }}>{col.title}</h4>
